...
from Server.core.respons_commands import ResponsCommands


setup_logger()


class AuthManager(JsonCommands, ResponsCommands):
...
Вы можете изменить свой никнейм с помощью соответствующей команды, сейчас он совпадает с юзернеймом.""")
    ...
    def update_nick(self, addr, nickname):
        ...
        self.Managers.auth_manager.auth_dict[username].update({
            'nickname': nickname
        })
        self.save_to_file(self.path, self.auth_dict)